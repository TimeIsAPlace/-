...
void* saTSP::threadFunc(void* param)
{
    ...
    while (t > t_min)
    {
        pthread_barrier_wait(&barrier1);//使用barrier进行同步
        mtx.lock();//加锁
...
void saTSP::SA_TSP()
{
    ...
    while (t > t_min)
    {
        if (tabu_Search(temp))
            continue;
        pthread_barrier_wait(&barrier1);//唤醒多线程
        pthread_barrier_init(&barrier1, NULL, worker_count + 1);//使多线程完成一次退火后沉睡
        pthread_barrier_wait(&barrier2);//多线程都完成后唤醒主线程
        t *= r; //退火  
        pthread_barrier_init(&barrier2, NULL, worker_count + 1);
    }
