...
void* saTSP::threadFunc(void* param)//线程函数
{
    ...
    for (int i = L / worker_count * rank; i < L / worker_count * (rank + 1); i++)//每个线程完成L / worker_count次循环
    {
        getNewSolution(temp);
...
void saTSP::SA_TSP()
{
    ...
    while (t > t_min)
    {
        ...
        threadParam_t* param = new threadParam_t[worker_count];//定义线程和参数
        ...
        for (int t_id = 0; t_id < worker_count; t_id++)
            pthread_create(&handles[t_id], NULL, threadFunc, (void*)(&param[t_id]));//动态线程每个循环都要创建
        for (int t_id = 0; t_id < worker_count; t_id++)
