...
void saTSP::SA_TSP()
{
    ...
    while (t > t_min)
    {
        ...
        if (rank == 0)
        {
            for (int i = 1; i < size; i++)
            {
                MPI_Send(&bestone.length, 1, MPI_FLOAT, i, i, MPI_COMM_WORLD);
                MPI_Send(&bestone.path, nCities, MPI_INT, i, i, MPI_COMM_WORLD);
            }
        }
        else
        {
            MPI_Recv(&bestone.length, 1, MPI_FLOAT, 0, rank, MPI_COMM_WORLD, &status);
            MPI_Recv(&bestone.path, nCities, MPI_INT, 0, rank, MPI_COMM_WORLD, &status);
        }
        t *= r; //退火  
    }
    MPI_Finalize();
