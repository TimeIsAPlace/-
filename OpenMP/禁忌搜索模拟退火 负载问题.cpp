...
void saTSP::SA_TSP()
{
    ...
    {
        for (int i = 0; i < 5; i++)
            memcpy(&temp_bestone[i], &bestone, sizeof(unit));
        if (tabu_Search(temp))
            continue;
#pragma omp parallel for num_threads(4),schedule(static,125)
        for (i = 0; i < L; i++)
        {
            getNewSolution(temp);
            if (Accept(bestone, temp, t))
            {
                memcpy(&temp_bestone[omp_get_thread_num()], &temp, sizeof(unit));
            }
            else
            {
                memcpy(&temp, &temp_bestone[omp_get_thread_num()], sizeof(unit));
            }
        }
#pragma omp master
        {
