...
double CalCulate_distance(double p1[2],double p2[2])
{
    double distance = sqrt(pow(p1[0] - p1[0], 2) + pow(p1[1] - p2[1], 2));
    return distance;
}
...
{
public:
    int init_point();                  // create matrix to storage the Distance each city
    void SA_TSP();
...
int saTSP::init_point() // create matrix to storage the Distance each city  
{
    int i = 0, j = 0;
    string line;
    double word;
...
void saTSP::CalCulate_length(unit& p)
{
    ...
    for (j = 1; j < nCities; j++)
    {
        p.length += CalCulate_distance(point_table[p.path[j - 1]],point_table[p.path[j]]);
    }
    p.length += CalCulate_distance(point_table[p.path[nCities - 1]], point_table[p.path[0]]);
}
